...
function follow_flows(dP::AbstractArray{T, 3}; niter::Int=200) where T
    ...
    for _ in 1:niter
        @inbounds for x in 1:W, y in 1:H
            ...
            # Clamp sicuro per evitare out-of-bounds nell'interpolazione
...
function compute_masks(dP::AbstractArray{T, 3}, cellprob::AbstractMatrix{T}; 
    ...
    # ✅ BFS a coda per convergenza garantita O(N)
    queue = Vector{Tuple{Int, Int}}()
    sizehint!(queue, sum(iscell))
    for (sy, sx, _) in seeds; push!(queue, (sy, sx)); end
...
function normalize99(img::AbstractArray)
    ...
    if ndims(img) == 2
        v = vec(img)
        p1 = quantile(v, 0.01f0)
        p99 = quantile(v, 0.99f0)
        denom = p99 - p1
        denom > 1f-6 && (out .= (img .- p1) ./ denom)
    elseif ndims(img) == 3
...
function pad_reflect(img::AbstractArray, pad_h::Int, pad_w::Int)
    ...
    if ndims(img) == 2
        ...
        out[1:H, 1:W] .= img
        if pad_h > 0
            for x in 1:W, y in 1:pad_h; out[H+y, x] = img[H-y+1, x]; end
        end
        if pad_w > 0
        ...
        out[1:H, 1:W, :] .= img
        if pad_h > 0
            for c in 1:C, x in 1:W, y in 1:pad_h; out[H+y, x, c] = img[H-y+1, x, c]; end
        end
        if pad_w > 0
            for c in 1:C, y in 1:H+pad_h, x in 1:pad_w; out[y, W+x, c] = out[y, W-x+1, c]; end
        end
        return out
    end
end
